...
import { callAndConfirm, EVMCallRequest, evmRead } from './common/evm'
import { generateCid, pinJsonToIPFS } from './common/ipfs'
...
if (
  ...
  throw new Error(
    'EVM_RPC and ATTESTEUR_ADDRESS and MULTIVAULT_ADDRESS and PRIVATE_KEY and CREATE_ATOM_ETH must be set',
  )
}
...
function depositAtomRequest(
  ...
  return {
    ...
    fnName: 'depositAtom',
    fnDeclaration: [
      'function depositAtom(address receiver, uint256 atomId) external payable returns (uint256)',
    ],
    args: [atomId],
    txParams: {
      gasLimit: 500000,
